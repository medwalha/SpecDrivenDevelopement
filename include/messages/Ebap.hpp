...
// Auto-generated from specs/euro_ctp_spec.pdf — §4.2.9 European Best Auction Price (EBAP)
// Spec version: 1.3   Generated: 2026-06-12
// TemplateID = 102 (Application, SchemaID=1). Payload = 117 bytes.
//
...
/// EBAP payload (§4.2.9). Five TrdRegTimestamps are always carried in fixed
/// order; their Type / Origin metadata is constant and therefore omitted from
/// the wire (see TrdRegTimestampType / TrdRegTimestampOrigin enums for the
/// implied values).
struct EbapStruct
{
    ...
    types::MdEntryType             m_MDEntryType;            ///< 1B — 'Q' or 'J' (char enum).
    ...
    types::InternalIdentifier      m_MDEntryID;              ///< 8B — EBAP identifier.
...
static_assert(sizeof(EbapStruct) == 117,
              "EbapStruct size mismatch with spec (117 bytes)");
static_assert(std::is_standard_layout<EbapStruct>::value,
              "EbapStruct must be standard-layout");
static_assert(std::is_trivially_copyable<EbapStruct>::value,
              "EbapStruct must be trivially copyable");

/// European Best Auction Price (EBAP) application message (§4.2.9).
class Ebap
{
    MessageHeader   const   m_MessageHeader;
    EbapStruct      const & m_Struct;

public:
    static constexpr unsigned int TYPE = MSG_ID_EBAP;
    static constexpr unsigned int SIZE = MessageHeader::SIZE + sizeof(EbapStruct);

    /// @param data Raw byte buffer that MUST start at the FramingHeader and
    ///             remain alive for the lifetime of this Ebap instance.
    explicit Ebap(InputData const & data)
    : m_MessageHeader(data)
    , m_Struct(*reinterpret_cast<const EbapStruct*>(data.data() + MessageHeader::SIZE))
    {
    }
...
inline std::ostream& operator<<(std::ostream& out, Ebap const& obj)
{
    obj.dump(out);
    return out;
}
